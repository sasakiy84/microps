...
ip_print(const uint8_t *data, size_t len)
{
    ...
    fprintf(stderr, "     offset: 0x%04x [flags=%x, offset=%u]\n", offset, (offset & 0xe000) >> 13, offset & IP_HDR_OFFSET_MASK);
...
ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    hlen = (hdr->vhl & 0x0f) << 2;
    if (len < hlen) {
        errorf("haeder length error: len=%zu < hlen=%u", len, hlen);
        return;
    }
...
ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    id = random16();
    plen = ip_build_packet(protocol, data, len, id, 0, src, dst, buf, sizeof(buf));
    if (plen == -1) {
